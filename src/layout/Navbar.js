...
import { AVATAR1, LOGO1, BASE_URL, AVATAR2, AVATAR3 } from "../utils/constants";
...
const Navbar = () => {
  ...
  useEffect(() => {
    setProfilePhoto(user?.photoURL || AVATAR1);
  }, [user]);
  ...
  return (
    <>
      <div
        className={`navbar ${
          showNavList && !isLargeScreen ? "h-[115px]" : "h-[70px]"
        }   fixed top-0 w-full px-4 md:px-12 py-3 text-white ${
          !isLargeScreen && "ddd"
        }`}
        style={{
          zIndex: 9999999,
          backgroundColor: (!isLargeScreen && "rgba(0, 0, 0, 0.5)") || "black",
          
          backgroundImage: `linear-gradient(180deg, #141414 ${navbarOpacity}%,transparent)`
            ...
            <div className="profile-dropdown relative" ref={dropdownRef}>
              ...
              {isOpen && (
                <div className="bg-black/95 absolute z-50 right-0 top-10 min-w-[170px] pt-2 border border-gray-900 rounded-md">
                  ...
                  >
                    <img src= {`${AVATAR1}`}  alt="avatar" className="w-5 h-5"></img>
                    
                    <div className="title">Divyansh</div>
                  </a>
                  <a
                    href="#!"
                    className="flex items-center px-4 py-2 gap-3 text-xs text-slate-500 hover:text-white"
                  >
                    <img src= {`${AVATAR2}`}  alt="avatar" className="w-5 h-5"></img>
                  ...
                  <Link
                    ...
                    className="flex items-center px-4 py-2 gap-3 text-xs text-slate-500 hover:text-white"
                  >
                    <img src= {`${AVATAR3}`}  alt="avatar" className="w-5 h-5"></img>
