    ...
    <div className='bg-black/70 w-full sm:w-[450px] m-auto px-6 md:px-16 py-8 md:py-12 mx-4 sm:mx-auto flex items-center rounded-lg'>
      <div className="w-full">
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="my-2">
            <button onClick={handleSignIn} disabled={loadingBtn ? true : false} className="flex items-center justify-center px-4 py-3 mt-4 bg-blue-500 hover:bg-blue-600 text-white w-full rounded-[4px] disabled:bg-blue-800">
              {
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex gap-2 items-center">
            <input id="rememberPswrd" type='checkbox' className="appearance-none rounded-sm w-4 h-4 bg-[#333] relative flex items-center justify-center checked:bg-[#4d4c4c] checked:text-white checked:before:content-['✓'] before:absolute cursor-pointer " />
            <label htmlFor='rememberPswrd' className='text-xs cursor-pointer'>Remember Password</label>
          </div>
          <div className='help'>
            <a href='#!' className='text-xs hover:text-white'>Need Help?</a>
          </div>
        </div>
        <div className="text-md mt-8 flex items-center gap-2">
          ...
          <Link to={PAGE.SIGNUP} className='text-white text-sm hover:text-gray-500'>Sign up now</Link>
        </div>
      </div >
    </div >
  )
